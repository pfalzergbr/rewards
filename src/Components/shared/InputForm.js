...
const InputForm = (props) => {
  ...
  const [
    ...
    stepImportanceUp,
    stepImportanceDown,
  ] = useInputScale(['A', 'B', 'C', 'D', 'E']);
  const [currentCoinValue, stepCoinValueUp, stepCoinValueDown] = useInputScale([
    1,
    2,
    3,
    4,
  ...
  const handleSubmit = (event) => {
    ...
    if (formName === 'todo-form') {
      ...
      if (input.length === 0) return toast.warning('Please enter a todo text!');
    ...
    <form onSubmit={handleSubmit} className='form '>
      <div className='form-wrapper'>
        <div className='input-wrapper'>
          {formName === 'todo-form' ? (
            <Stepper
              stepUp={stepImportanceUp}
              stepDown={stepImportanceDown}
              currentValue={currentImportance}
              stepperVariant='importance'
            />
          ) : (
            <Stepper
              stepUp={stepCoinValueUp}
              stepDown={stepCoinValueDown}
              currentValue={currentCoinValue}
              stepperVariant='coin-value'
            />
          )}
        ...
        <button type='submit' className={`btn btn-primary btn--${formName}`}>
          {buttonText}
        </button>
      </div>
    </form>
